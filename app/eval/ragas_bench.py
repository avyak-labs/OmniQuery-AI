            ...
            results.append(
                {
                    ...
                    "document_name": d.get("metadata", {}).get(
                        "document_name", "OmniCorp Document"
                    ),
                    "chunk_index": rank + 1,
                    "content": d.get("content", ""),
...
async def run_pipeline_for_eval(
    ...
    for idx, item in enumerate(questions, start=1):
        ...
        print(
            f"  [{idx:02d}/{len(questions):02d}] Processing ({category}): '{q[:55]}...'"
        )
        ...
        try:
            ...
            print(
                f"    ⚠️ Live DB retrieval failed ({e}). Using offline seed corpus fallback."
            )
            raw_chunks = _offline_seed_retrieval(q, top_k=3)
        ...
        chunk_texts = [
            c.get("content", "") if isinstance(c, dict) else str(c) for c in raw_chunks
        ]
...
def calculate_ragas_scores(eval_records: list[dict[str, Any]]) -> dict[str, float]:
    ...
    if gemini_key:
        try:
            ...
            print(
                f"⚠️ [RAGAS BENCHMARK] Ragas live evaluation encountered an error ({e}). Using robust fallback scoring."
            )
