...
def test_chunk_documents():
    ...
    sample_docs = [
        {
            "content": "A" * 1200,
            "metadata": {"document_name": "TestDoc.pdf", "document_id": "test_doc"},
        }
    ]
...
def test_langgraph_intent_router():
    ...
    # Test SQL routing
    sql_state = classify_intent_node(
        {"query": "How many total orders were placed last month?", "query_type": None}
    )
    assert sql_state["query_type"] == "sql"

    # Test RAG routing
    rag_state = classify_intent_node(
        {
            "query": "What is the company remote work policy on internet reimbursement?",
            "query_type": None,
        }
    )
    ...
    direct_state = classify_intent_node(
        {"query": "Hello, who are you?", "query_type": None}
    )
