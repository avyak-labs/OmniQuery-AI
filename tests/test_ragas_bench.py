...
def test_ground_truth_dataset_integrity():
    ...
    assert len(data) >= 10, (
        f"Expected at least 10 evaluation test cases, found {len(data)}"
    )
    ...
    for item in data:
        ...
        assert not missing, (
            f"Item {item.get('id', 'unknown')} missing required keys: {missing}"
        )
        assert len(item["question"].strip()) > 10, f"Question too short in {item['id']}"
        assert len(item["ground_truth"].strip()) > 10, (
            f"Ground truth too short in {item['id']}"
        )
...
def test_ragas_scores_meet_thresholds():
    ...
    sample_eval_records = [
        ...
        },
        {
            ...
            "contexts": [
                "Error ERR_AUTH_401 indicates invalid bearer token credentials."
            ],
    ...
    assert scores["faithfulness"] >= 0.85, (
        f"Faithfulness dropped below threshold: {scores['faithfulness']}"
    )
    assert scores["answer_relevance"] >= 0.80, (
        f"Answer relevance dropped below threshold: {scores['answer_relevance']}"
    )
    assert scores["context_precision"] >= 0.75, (
        f"Context precision dropped below threshold: {scores['context_precision']}"
    )
